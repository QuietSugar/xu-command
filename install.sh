...
XU_COMMAND_RELEASE_URL="${XU_COMMAND_URL:-https:/1.1.1.1/1.tar.gz}"
XU_COMMAND_INSTALL_ROOT_PATH=${HOME}"/.xu-command/"
XU_COMMAND_NAME="xu-command"
xc_env_path=${XU_COMMAND_INSTALL_ROOT_PATH}"env"
...
function copy_sh_file(){
  ...
  echo "copy_sh_file in $command_path"
  pushd "${command_path}"
...
if [ -d "$HOME/.xu-command/source" ] ; then
  FILES=$(find "${HOME}/.xu-command/source" -name '*.sh' | sort)
...
function uninstall() {
  ...
  rm -rf ${XU_COMMAND_INSTALL_ROOT_PATH}
}
